...
const PreviewLoader = ({ onComplete, onProgress }) => {
  ...
  useEffect(() => {
    ...
    const animation = animate(progress, 100, {
      ...
      onUpdate: (latest) => {
        ...
        // Remove backdrop filter at 80% to prevent dark blur
        if (latest >= 80 && !removeBackdrop) {
  ...
  // Remove backdrop filter when progress >= 80% or exiting to prevent dark blur
  const finalBackdropFilter = (isExiting || removeBackdrop) ? 'none' : backdropFilter;
